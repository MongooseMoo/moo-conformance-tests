...
- [x] CON-001 listener_handler_do_login_command: `listen(handler, port, options)` dispatches login hooks on that handler.
- [x] CON-002 listener_handler_do_command: post-login command hooks dispatch on the connection's handler.
- [x] CON-003 listener_handler_do_blank_command: trusted-proxy blank-line hook dispatches on the listener handler.
...
- [x] CON-010 flush_command_flushes_pending_input: connection `flush-command` flushes queued input.
...
- [x] GAP-002 transport_telnet_iac_binary: telnet IAC and binary-mode line parsing match Toast.
- [x] GAP-003 output_order_disconnect: output buffer ordering during disconnect matches Toast.
