...
GENERATOR = "tools/generate_toast_semantic_matrices.py"
...
def comparison(left: Value, right: Value) -> int | str:
    ...
    left_order = left.order.lower() if isinstance(left.order, str) else left.order
    right_order = right.order.lower() if isinstance(right.order, str) else right.order
    return (left_order > right_order) - (left_order < right_order)


def directional_relations(comparison_result: int | str) -> list[int | str]:
    if comparison_result == "E_TYPE":
        return ["E_TYPE"] * 4
    return [
